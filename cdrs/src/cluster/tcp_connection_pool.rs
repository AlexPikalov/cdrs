...
use crate::transport_builder_trait::CDRSTransportBuilder;
use crate::transport_tcp::{TcpTransportBuilder, TransportTcp};
use authenticators::Authenticator;
use cassandra_proto::{
  error,
  frame::{parser::parse_frame, Frame, IntoBytes, Opcode},
};
...
pub type TcpConnectionPool<A> = Pool<ConnectionsManager<A, TransportTcp, TcpTransportBuilder>>;
...
pub fn new_tcp_pool<'a, A: Authenticator + Send + Sync + 'static>(
  ...
  let tcp_transport_builder = TcpTransportBuilder::new(node_config.addr.to_string());
  let manager = ConnectionsManager::new(tcp_transport_builder, node_config.authenticator);
...
pub struct ConnectionsManager<A, T: CDRSTransport, B: CDRSTransportBuilder<T>> {
  transport_builder: B,
  auth: A,
  phantom_transport: PhantomData<T>,
}

impl<A, T: CDRSTransport, B: CDRSTransportBuilder<T>> ConnectionsManager<A, T, B> {
  pub fn new(transport_builder: B, auth: A) -> Self {
    ConnectionsManager {
      transport_builder,
      auth,
...
impl<
    A: Authenticator + 'static + Send + Sync,
    T: CDRSTransport + 'static + Send + Sync,
    B: CDRSTransportBuilder<T> + 'static + Send + Sync,
  > ManageConnection for ConnectionsManager<A, T, B>
{
  type Connection = RefCell<T>;
  type Error = error::Error;

  fn connect(&self) -> Result<Self::Connection, Self::Error> {
    let transport = RefCell::new(self.transport_builder.create()?);
