...
impl<'a, L> Session<L> {
  ...
  pub fn listen<A: Authenticator + 'static + Sized, T: CDRSTransport + 'static + Sized>(
    &self,
    transport: T,
    authenticator: A,
    events: Vec<SimpleServerEvent>,
  ) -> error::Result<(Listener<RefCell<T>>, EventStream)> {
    let compression = self.get_compressor();
    let transport_cell = RefCell::new(transport);

    startup(&transport_cell, &authenticator)?;
