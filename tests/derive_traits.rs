...
#[test]
#[cfg(feature = "e2e-tests")]
fn update_list_with_udt_with_values() {
  let drop_table_cql = "DROP TABLE IF EXISTS cdrs_test.update_list_with_udt_2";
  let drop_type_cql = "DROP TYPE IF EXISTS cdrs_test.update_list_with_udt_type";
  let create_type_cql = "CREATE TYPE cdrs_test.update_list_with_udt_type (id uuid,
    text text)";
  let create_table_cql =
    "CREATE TABLE IF NOT EXISTS cdrs_test.update_list_with_udt_2 \
     (id uuid PRIMARY KEY, udts_set set<frozen<cdrs_test.update_list_with_udt_type>>)";
  let session = setup_multiple(&[
    drop_table_cql,
    drop_type_cql,
    create_type_cql,
  ...
  let cql = "INSERT INTO cdrs_test.update_list_with_udt_2 \
             (id, udts_set) VALUES (?, ?)";
  session
    .query_with_values(cql, row_struct.clone().into_query_values())
    .expect("insert");
  ...
  let query = session
    .prepare("UPDATE cdrs_test.update_list_with_udt_2 SET udts_set = udts_set + ? WHERE id = 5bd8877a-e2b2-4d6f-aafd-c3f72a6964cf")
    .expect("prepare query");
  let vals = query_values!(MyUdt {
    id: Uuid::parse_str("68f49fa5-934b-4aff-8a87-f3a32872a6ba").expect("udt id"),
    text: "abc".into(),
  });
  session.exec_with_values(&query, vals).expect("update set");
