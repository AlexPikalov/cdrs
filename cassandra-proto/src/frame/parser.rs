...
pub fn parse_frame<E>(cursor_cell: &RefCell<Read>, compressor: &Compressor<CompressorError=E>) -> error::Result<Frame> where E: std::error::Error {
    ...
    let full_body = if flags.iter().any(|flag| flag == &Flag::Compression) {
        compressor.decode(body_bytes).map_err(|err| error::Error::from(err.description()))?
    } else {
        body_bytes
    };
